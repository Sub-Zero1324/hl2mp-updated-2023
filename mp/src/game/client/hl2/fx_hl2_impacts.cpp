...
// Sub-Zero Added for gauss impact effects Single Fire
//-----------------------------------------------------------------------------
// Purpose: Purpose: Handle Gauss Glow Effects Single Fire
// Input  : &origin - 
//			&normal - 
//			scale - 
//-----------------------------------------------------------------------------
void FX_GaussGunImpactYellow( const Vector &origin, const Vector &normal, float scale )
{
	...
	FX_AddQuad( origin, 
				...
				Vector( 1.0f, 1.0f, 1.0f ), 
				5.05f, 
				"decals/yellowglowfade",
				(FXQUAD_BIAS_SCALE|FXQUAD_BIAS_ALPHA) );
}

//-----------------------------------------------------------------------------
// Purpose: Handle GaussGun Glow Effects Single Fire
//-----------------------------------------------------------------------------
void ImpactGaussGunYellowCallback( const CEffectData &data )
{
	VPROF_BUDGET( "ImpactGaussGunYellowCallback", VPROF_BUDGETGROUP_PARTICLE_RENDERING );
	...
	if ( Impact( vecOrigin, vecStart, iMaterial, iDamageType, iHitbox, pEntity, tr, IMPACT_NODECAL | IMPACT_REPORT_RAGDOLL_IMPACTS ) )
	{
		FX_GaussGunImpactYellow( vecOrigin, tr.plane.normal, 2 );
	}
#else
	FX_GaussGunImpactYellow( vecOrigin, tr.plane.normal, 1 );
#endif
}

DECLARE_CLIENT_EFFECT( "GaussYellowGlow", ImpactGaussGunYellowCallback );

// Sub-Zero Added for gauss impact effects Single Fire

// Sub-Zero Added for gauss impact effects Charged Fire
//-----------------------------------------------------------------------------
// Purpose: Purpose: Handle Gauss Glow Effects Charged Fire
// Input  : &origin - 
//			&normal - 
//			scale - 
//-----------------------------------------------------------------------------
void FX_GaussGunImpactRed( const Vector &origin, const Vector &normal, float scale )
{
...
//-----------------------------------------------------------------------------
// Purpose: Handle GaussGun Glow Effects Charged Fire
//-----------------------------------------------------------------------------
void ImpactGaussGunRedCallback( const CEffectData &data )
{
	VPROF_BUDGET( "ImpactGaussGunRedCallback", VPROF_BUDGETGROUP_PARTICLE_RENDERING );
	...
	if ( Impact( vecOrigin, vecStart, iMaterial, iDamageType, iHitbox, pEntity, tr, IMPACT_NODECAL | IMPACT_REPORT_RAGDOLL_IMPACTS ) )
	{
		FX_GaussGunImpactRed( vecOrigin, tr.plane.normal, 2 );
	}
#else
	FX_GaussGunImpactRed( vecOrigin, tr.plane.normal, 1 );
#endif
}

DECLARE_CLIENT_EFFECT( "GaussRedGlow", ImpactGaussGunRedCallback );
// Sub-Zero Added for gauss impact effects Charged Fire
