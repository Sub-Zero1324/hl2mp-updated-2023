...
void C_BaseAnimating::ProcessMuzzleFlashEvent()
{
	...
	if ( muzzleflash_light.GetBool() )
	{
		...
		{
			// Sub-Zero Muzzle Flash Lighting Fix
	/*		Vector vAttachment;
			QAngle dummyAngles;
...
bool C_BaseAnimating::DispatchMuzzleEffect( const char *options, bool isFirstPerson )
{
	...
	if ( token[0] ) 
	{
		...
		if ( Q_stricmp( token, "COMBINE1" ) == 0 )
		{
			weaponType = MUZZLEFLASH_COMBINE;
		}
		else if ( Q_stricmp( token, "SMG11" ) == 0 )
		{
			weaponType = MUZZLEFLASH_SMG1;
		}
		else if ( Q_stricmp( token, "PISTOL1" ) == 0 )
		{
			weaponType = MUZZLEFLASH_PISTOL;
		}
		else if ( Q_stricmp( token, "SHOTGUN1" ) == 0 )
		{
			weaponType = MUZZLEFLASH_SHOTGUN;
		}
		else if ( Q_stricmp( token, "3571" ) == 0 )
		{
			weaponType = MUZZLEFLASH_357;
		}
		else if ( Q_stricmp( token, "RPG1" ) == 0 )
		{
