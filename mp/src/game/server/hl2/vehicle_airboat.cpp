...
static ConVar sk_airboat_recharge_rate("sk_airboat_recharge_rate", "40");
...
//static ConVar sk_plr_dmg_airboat("sk_plr_dmg_airboat", "3");
//static ConVar sk_npc_dmg_airboat("sk_npc_dmg_airboat", "3");
ConVar sk_dmg_airboat("sk_dmg_airboat", "2");
...
private:
	...
	void			FireGun();
	void			FireCannon(Vector* pVecAimPoint);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPropAirboat::FireCannon(Vector* pVecAimPoint)
{
...
#ifndef CLIENT_DLL         
	CTakeDamageInfo dmgInfo(this, GetDriver(), sk_dmg_airboat.GetFloat(), DMG_AIRBOAT);
#endif

	if (pHit != NULL)
	{
#ifndef CLIENT_DLL
		CalculateBulletDamageForce(&dmgInfo, m_nBulletType, vecEndPos, trace.endpos, 7.0f * 5.0f);
		pHit->DispatchTraceAttack(dmgInfo, vecEndPos, &trace);
#endif
	}
...
void CPropAirboat::FireGun()
{
	...
	//ShadowSource - Information: Thanks to Sub-Zero, we have working hit effects from the tau cannon.
	CDisablePredictionFiltering disabler;
	...
	*/
	...
	DoMuzzleFlash();
	DispatchParticleEffect("hl2mmod_muzzleflash_npc_ar2", PATTACH_POINT_FOLLOW, this, "muzzle", true);
	FireCannon(&vecAimPoint);
