...
#define	MAX_JEEP_CHARGE_TIME		3.8
...
ConVar sk_plr_dmg_gauss("sk_plr_dmg_gauss", "10");
//=============================================================================
//
// Jeep water data.
//
...
void CPropJeep::Precache(void)
{
	...
	PrecacheScriptSound("PropJeep.OverCharge");
...
//			useMuzzle - 
//-----------------------------------------------------------------------------
void CPropJeep::DrawBeam2(const Vector& startPos, const Vector& endPos, float width, bool useMuzzle)
{

	int iAttachment = GetTracerAttachment();
	UTIL_ParticleTracer("hl2mmod_tracer_gaussrifle_tracer_buggy1", startPos, endPos, iAttachment, true);
...
#ifndef CLIENT_DLL         
	CTakeDamageInfo dmgInfo(this, GetDriver(), sk_plr_dmg_gauss.GetFloat(), DMG_SHOCK | DMG_BULLET);
#endif
...
void CPropJeep::FireChargedCannon(void)
{
	...
	StopChargeSound();
	StopSound("Jeep.GaussCharge");
	StopSound("PropJeep.OverCharge");
	...
	if (tr.DidHitWorld() && !(tr.surface.flags & SURF_SKY))
	{
		UTIL_ImpactTrace(&tr, m_nBulletType, "GaussRedGlow");
		UTIL_ImpactTrace(&tr, m_nBulletType, "HelicopterImpact");
	}
	...
	if (penetrated == true)
	{
		RadiusDamage(CTakeDamageInfo(this, GetDriver(), flDamage, DMG_SHOCK), tr.endpos, 200.0f, CLASS_NONE, NULL);

		Vector  newPos = tr.endpos + (aimDir * MAX_TRACE_LENGTH);
		trace_t beam_tr;
		Vector vecDest = tr.endpos + aimDir * MAX_TRACE_LENGTH;
		UTIL_TraceLine(tr.endpos, vecDest, MASK_SHOT, m_hPlayer, COLLISION_GROUP_NONE, &beam_tr); //Traces from back of first wall to second wall

		//  DrawBeam( tr.endpos, beam_tr.endpos, 9.6, false );
		DrawBeam2(tr.endpos, beam_tr.endpos, 9.6, false);
		DoWallBreak(tr.endpos, newPos, aimDir, &tr, m_hPlayer, true);
		RadiusDamage(CTakeDamageInfo(this, m_hPlayer, flDamage, DMG_SHOCK), tr.endpos, 10.0f, CLASS_PLAYER_ALLY, m_hPlayer);
	}
}
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
bool CPropJeep::DidPunchThrough(trace_t* tr)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPropJeep::DoWallBreak(Vector startPos, Vector endPos, Vector aimDir, trace_t* ptr, CBasePlayer* pOwner, bool m_bBreakAll) {
	trace_t* temp = ptr;
	if (m_bBreakAll) {
		...
		while (DidPunchThrough(ptr)) {
			...
			if (ptr->DidHitWorld() && ptr->surface.flags != SURF_SKY) {
				UTIL_ImpactTrace(ptr, GetAmmoDef()->DamageType(m_nBulletType), "ImpactGauss");
				UTIL_ImpactTrace(ptr, GetAmmoDef()->DamageType(m_nBulletType), "GaussRedGlow");
				UTIL_ImpactTrace(ptr, GetAmmoDef()->DamageType(m_nBulletType), "HelicopterImpact");
			}
			startPos = ptr->endpos;
...
void CPropJeep::ChargeCannon(void)
{
	...
	else
	{
		...
		if ((gpGlobals->curtime - m_flCannonChargeStartTime) > MAX_JEEP_CHARGE_TIME)
		{
			//Notify the player they're at maximum charge
			if (m_bChargeIndicated == false)
			{
				//EmitSound("PropJeep.OverCharge");
				m_bChargeIndicated = true;
			}
		}


		float flChargeAmount = (gpGlobals->curtime - m_flCannonChargeStartTime) / MAX_JEEP_CHARGE_TIME;
		if (flChargeAmount > 1.0f)
		{
			flChargeAmount = 1.0f;
			//EmitSound("PropJeep.OverCharge");
...
void CPropJeep::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	if (tr.m_pEnt == this && tr.hitgroup == JEEP_AMMOCRATE_HITGROUP)
	{
		...
		pPlayer->GiveAmmo(150, "GaussEnergy");
