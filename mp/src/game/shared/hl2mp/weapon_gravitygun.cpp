...
#define PHYSGUN_BEAM_SPRITE		"effects/redlaser1.vmt"
#define PHYSGUN_BEAM_GLOW		"effects/redflare.vmt"
#define	PHYSGUN_SKIN	1

class CWeaponLaserGun;

#ifdef CLIENT_DLL
CLIENTEFFECT_REGISTER_BEGIN( PrecacheEffectGravityGun2 )
CLIENTEFFECT_MATERIAL( "effects/redlaser1" )
...
ConVar sk_plr_dmg_egon( "sk_plr_dmg_egon", "0.1" );
//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
...
void CWeaponLaserGun::UseAmmo( int count )
{
...
void CWeaponLaserGun::OnRestore( void )
{
	BaseClass::OnRestore();
...
#ifndef CLIENT_DLL         
         CTakeDamageInfo dmgInfo( this, pOwner, sk_plr_dmg_egon.GetFloat(), DMG_BLAST | DMG_BULLET );
#endif
 
         if ( pHit != NULL )
        {
#ifndef CLIENT_DLL
                 CalculateBulletDamageForce( &dmgInfo, m_iPrimaryAmmoType, aimDir, tr.endpos, 1.0f * 0.0f  );
...
int CWeaponLaserGun::DrawModel( int flags )
{
	...
	if ( flags & STUDIO_TRANSPARENCY )
	{
		...
		IMaterial *pMat = materials->FindMaterial( "effects/redlaser1", TEXTURE_GROUP_CLIENT_EFFECTS );
		Vector color;
		color.Init(1,1,1);
		...
		//DrawBeamQuadratic( points[0], points[1], points[2], pObject ? 13/3.0f : 13/5.0f, color, -scrollOffset );
		DrawBeamQuadratic( points[0], points[1], points[2], 6, color, scrollOffset );
		DrawBeamQuadratic( points[0], points[1], points[2], 6, color, -scrollOffset );

		IMaterial *pMaterial = materials->FindMaterial( "effects/redflare", TEXTURE_GROUP_CLIENT_EFFECTS );
...
//		}
		...
		float scale = random->RandomFloat( 24, 5 ) * ( pObject ? 3 : 2 );
