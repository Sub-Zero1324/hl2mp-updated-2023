...
void CMissile::Precache( void )
{
	...
	PrecacheParticleSystem("hl2mmod_weapon_rpg_smoketrail");
}
...
void CMissile::CreateSmokeTrail( void )
{
	...
	if ( (m_hRocketTrail = RocketTrail::CreateRocketTrail()) != NULL )
	{
		...
		*/
		DispatchParticleEffect("hl2mmod_weapon_rpg_smoketrail", PATTACH_POINT_FOLLOW, this, "0", true);
	}
}
...
void CWeaponRPG::Precache( void )
{
	...
	PrecacheParticleSystem("hl2mmod_muzzleflash_rpg");
...
{
#ifndef CLIENT_DLL // Sub-Zero Added to fix RPG Lowering animation playing when the weapon is fired
	// Lower us if we're out of ammo
	if ( !HasAnyAmmo() )
		return true;
	#endif // Sub-Zero Added to fix RPG Lowering animation playing when the weapon is fired
...
void CWeaponRPG::PrimaryAttack( void )
{
	...
	DispatchParticleEffect("hl2mmod_muzzleflash_rpg", PATTACH_POINT_FOLLOW, pPlayer->GetViewModel(), "muzzle", true);
//#ifdef CLIENT_DLL
	//DispatchParticleEffect("hl2mmod_muzzleflash_rpg", PATTACH_POINT_FOLLOW, this, "muzzle", true);
//#endif
	...
	UTIL_ParticleTracer("hl2mmod_muzzleflash_rpg", tr.startpos, tr.endpos, iEntIndex, iAttachment, true);
...
{
CDisablePredictionFiltering disabler; // Sub-Zero Added to fix RPG Lowering animation playing when the weapon is fired
#ifndef CLIENT_DLL // Sub-Zero Added to fix RPG Lowering animation playing when the weapon is fired
