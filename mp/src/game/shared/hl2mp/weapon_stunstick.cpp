...
// MuzzleLight
//---------------------------------------------------------
void CWeaponStunStick::MuzzleLight(void)
{
#ifdef CLIENT_DLL
...
void CWeaponStunStick::SetStunState( bool state )
{
	...
	if ( m_bActive )
	{
		...
		//g_pEffects->Sparks( vecAttachment ); // Sub-Zero Removed to Fix Stunstick spark issues

		//FIXME: END - Move to client-side

		EmitSound( "Weapon_StunStick.Activate" ); // Sub-Zero Removed to Fix Stunstick spark issues
	}
	else
	{
		EmitSound( "Weapon_StunStick.Deactivate" ); // Sub-Zero Removed to Fix Stunstick spark issues
	}
}
