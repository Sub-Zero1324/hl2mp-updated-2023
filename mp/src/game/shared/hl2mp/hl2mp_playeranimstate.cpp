...
void CHL2MPPlayerAnimState::ComputePoseParam_MoveYaw(CStudioHdr* pStudioHdr)
{
	...
	//QAngle	angles = GetBasePlayer()->GetLocalAngles(); // Sub-Zero - Removed to fix Wonky leg animation
	//float ang = angles[ YAW ]; // Sub-Zero - Removed to fix Wonky leg animation
	float ang = m_flEyeYaw; // Sub-Zero - Added to fix Wonky leg animation
	if (ang > 180.0f)
	{
		ang -= 360.0f;
	}
