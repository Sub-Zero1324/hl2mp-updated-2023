...
bool CMultiPlayerAnimState::HandleVehicle(Activity& idealActivity)
{
	...
	{
		if (Q_stristr(pVehicle->GetClassname(), "airboat"))
		{
			//ShadowSource - Information: Once you have created your custom player animations, set sequences in the .qc file and remove/replace the ACT idle below.
...
//	}
		...
		{
		//	GetBasePlayer()->GetVehicle()->GetVehicleEnt();


			if (!Q_stristr(GetBasePlayer()->GetVehicle()->GetVehicleEnt()->GetClassname(), "jeep"))
			{
				idealActivity = ACT_HL2MP_DRIVE_AIRBOAT; //ACT_HL2MP_SIT_IN_VEHICLE;

				if (GetBasePlayer()->m_nButtons & IN_MOVELEFT)
				{
					idealActivity = ACT_HL2MP_DRIVE_AIRBOAT_LEFT; //ACT_HL2MP_SIT_IN_VEHICLE;
				}


				if (GetBasePlayer()->m_nButtons & IN_MOVERIGHT)
				{
		...
		if (GetBasePlayer()->IsInAVehicle() && GetBasePlayer()->GetVehicle())
		{
			...
			if (!Q_stristr(GetBasePlayer()->GetVehicle()->GetVehicleEnt()->GetClassname(), "airboat"))
			{
				idealActivity = ACT_HL2MP_DRIVE_JEEP; //ACT_HL2MP_SIT_IN_VEHICLE;
			...
			// if(pPlayer->IsInAVehicle() && FClassnameIs(pPlayer->GetVehicle()->GetVehicleEnt(),"prop_vehicle_??"))
			// {
